...
Interactive components (Button, any component with a clickable surface) use `aria-disabled="true"` instead of the native `disabled` attribute. The reason: native `disabled` removes an element from the keyboard tab order, which fails WCAG 2.1.1 (Keyboard) and breaks the expected experience for keyboard and screen-reader users.
