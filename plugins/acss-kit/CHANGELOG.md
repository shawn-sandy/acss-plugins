...
- **`.acss-target.json` schema is now additive with a `stack` block** — `{ framework, bundler, cssPipeline, tsconfig, entrypointFile, detectedAt }`. Existing `componentsDir` / `utilitiesDir` consumers are unaffected; the detectors and verifiers degrade gracefully when the block is absent (they emit a reason pointing back to `detect_stack.py`).
