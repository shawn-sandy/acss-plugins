...
- **Pilot `component-creator` skill and `/kit-create` command** — natural-language creator mode for components. Describe a UI element ("primary pill button that says 'Add to cart'", "large outline button labeled Sign in", "danger button for 'Delete account'") and the skill parses color / size / variant / block / disabled / children, vendors the component via `/kit-add` if missing, and emits a paste-ready TSX snippet (default) or a standalone `src/components/<Name>.tsx` file. v0.1 is scoped to **Button** only — other components surface a v0.2 placeholder. Variant vocabulary mirrors `references/components/button.md`; ambiguous parses (unresolved color, conflicting synonyms, accessibility-risky size combos) halt with `AskUserQuestion` rather than silently defaulting. Refinement turns ("make it larger", "swap to secondary", "drop the full width") merge into the in-memory spec and re-emit; an 8-row validation matrix blocks broken combinations (icon-without-glyph, two-colour conflicts, empty children, > 60-char labels) before any disk write. `variant: "icon"` is intentionally excluded from v0.1 and deferred to the dedicated `IconButton` component in v0.2.
