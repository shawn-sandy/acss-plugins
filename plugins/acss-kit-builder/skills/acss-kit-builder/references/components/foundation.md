...
> **Verified against fpkit source:** `@fpkit/acss@6.5.0` (closest tagged ref to npm `6.6.0`). The vendored `UI` component is the polymorphic React primitive that every other kit-builder component imports as `import UI from '../ui'`. **Intentional preservation**: this is the one file copied verbatim from fpkit at first `/kit-add` run — the polymorphic-component pattern with ref-forwarding has subtle TypeScript shape requirements (the `as unknown as UIComponent` double-cast in particular) that benefit from being a single canonical implementation rather than a re-derived one.
>
> **Phase 0 audit finding (acss-plugins refactor 0.2.0):** `assets/foundation/ui.tsx` is also referenced by `acss-component-specs/skills/.../frameworks/react.md`. The `assets/foundation/ui.tsx` file remains in place as the canonical first-run-copy source until the cross-plugin coordination with `acss-component-specs` is complete; this markdown spec exists alongside as the documented embedded-markdown shape for the foundation, not as a replacement.
