> **Legacy reference** — This bundled reference is superseded by `acss-component-specs/specs/alert.md` when that plugin is installed. Prefer the spec-based workflow: `/spec-add alert` then `/spec-render alert`. The spec wins when both are present (kit-builder B0 bridge).
