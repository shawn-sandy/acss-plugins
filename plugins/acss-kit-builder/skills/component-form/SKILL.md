...
```
{
  ...
  type: 'text' | 'email' | 'password' | 'tel' | 'url' | 'textarea' | 'select' | 'checkbox',
...
If the user's description specifies a type the skill doesn't support directly (e.g. file upload, date picker), surface that limitation explicitly: "The kit-builder form components support text/email/password/tel/url, textarea, select, and checkbox out of the box. For file uploads or date pickers, use the native `<input type="file" />` or `<input type="date" />` directly — they don't need a kit-builder wrapper."
...
import Field from '../components/fpkit/field/field'
import Input from '../components/fpkit/input/input'
{{IMPORT_CHECKBOX_IF_NEEDED}}
...
| `{{NAME_KEBAB}}` | The kebab-case form name (e.g. `signup-form`) |
| `{{HEADING}}` | The form's visible heading (e.g. `Create your account`) |
| `{{SUBMIT_LABEL}}` | The submit button label (e.g. `Create account`) |
| `{{FIELD_TYPES}}` | One TypeScript line per field: `  fieldName: string` (or `boolean` for checkbox) |
| `{{FIELDS}}` | The rendered field elements — see Field Renderers below |
| `{{IMPORT_CHECKBOX_IF_NEEDED}}` | `import Checkbox from '../components/fpkit/checkbox/checkbox'` if any field has `type: 'checkbox'`; otherwise omit |
...
### Text-like inputs (text, email, password, tel, url)

```tsx
<Field labelFor="{{name}}" label="{{label}}">
  <Input
    id="{{name}}"
    name="{{name}}"
    type="{{type}}"
...
```tsx
<Field labelFor="{{name}}" label="{{label}}">
  <select id="{{name}}" name="{{name}}" {{REQUIRED_ATTR}} aria-required={{required}}>
...
|--------------|-----------------|
| text, email, password, tel, url, textarea, select | `string` |
| checkbox | `boolean` |
