...
The generated form needs `Field`, `Input`, `Button`, and (when a checkbox field is present) `Checkbox`. Where those come from depends on the project's component source — Step B branches on `detect_component_source.py` output before deciding whether to probe the local file system.

### B1. Resolve the component source

Run `python3 plugins/acss-app-builder/scripts/detect_component_source.py <project_root>` (the same script `acss-app-builder` shared preflight invokes). Parse the JSON output. The relevant keys are `source`, `componentsDir`, and (when source is `npm`) `deprecated` + `sunsetVersion`.

### B2. Branch on source

**`source: "npm"` *(deprecated path)*** — `Field`, `Input`, `Checkbox`, and `Button` are provided by the `@fpkit/acss` package; **skip the local-file probe** and proceed to Step C. The TSX Template's `{{IMPORT_SOURCE:Field,Input,Checkbox,Button}}` placeholder will resolve to the npm-style combined import. Do not run `/kit-add` — that would force the user off a documented soft-deprecation window. Carry the JSON's `deprecated`/`sunsetVersion` flags into Step G's post-generation summary so the migration nudge appears.

**`source: "generated"`** — proceed to B3 to probe `<componentsDir>` for the required local files.

**`source: "none"`** — error before generation: print the install hint that `acss-app-builder/skills/.../references/component-source.md` recommends (`/kit-add field input` plus `checkbox` if needed, or `npm install @fpkit/acss` as the deprecated alternative). Halt.

### B3. Probe required components *(generated source only)*
...
### B4. Generate missing components *(generated source only)*
...
This step does **not** apply when source is `npm` — those projects keep the package and skip the vendoring step entirely until they choose to migrate.
...
| `{{IMPORT_SOURCE:Field,Input,Checkbox,Button}}` | Source-aware import block. Resolves per `detect_component_source.py` output — see "Component-source-aware imports" subsection below. Drop `Checkbox` from the placeholder list when no checkbox field is present. `Button` is always included (the submit button uses it). |
...
Step B already determined the component source (`generated` | `npm`) by reading `.acss-target.json` and (for the generated path) probing for required components. Use that source when expanding `{{IMPORT_SOURCE:Field,Input,Checkbox,Button}}`:
...
2. **For `source=generated`**: emit one import per component referenced in the placeholder, plus the matching SCSS imports. Drop `Checkbox` lines if no checkbox field is present; `Button` is always included:
   ```tsx
...
3. **For `source=npm`**: emit a single combined import (drop `Checkbox` if not needed). Per-form SCSS imports are unnecessary — npm projects load `@fpkit/acss/styles` once at app entry:
   ```tsx
   import { Field, Input, Checkbox, Button } from '@fpkit/acss'
   ```
4. **For `source=none`**: error — Step B should have already halted with an install hint before generation reaches this point.
...
**Submit button**
- Renders the kit-builder `Button` with `disabled={submitting}`. `Button` propagates `aria-disabled="true"` to the underlying element while keeping it in the tab order — screen readers announce "dimmed/unavailable" without focus loss (WCAG 2.1.1).
